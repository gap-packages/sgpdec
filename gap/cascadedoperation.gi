...
depfuncval := cascop![Length(coords)](coords{[1..(Length(coords)-1)]});
  ...
  if IsTransformation(depfuncval) and RankOfTransformation(depfuncval)=1 then
    ...
    if coords[Length(coords)] = 0 then #TODO potential problem here when acting on 0 as a general transformation may reduce the set
      return 0;
    else
      return coords[Length(coords)] ^ depfuncval; # ^ should be the right action
    fi;
  fi;    
end;
...
SGPDEC_CascadedAction := function(coords, cascop)
  return List([1..Size(coords)], x -> SGPDEC_ActionOnLastCoord(coords{[1..x]},cascop));
end;

# for creating the identity cascaded permutation in a given family of cascaded product elements
# no dependencies, just the component's identtiy is returned 
...
local deps,i,tmp, level,arg,action;
  ...
  #now putting the actions there - this may still give identity and reduce the number of dependencies
  deps := List(deps, arg -> [arg, Random(cstr[Length(arg)+1])]);
  return  DefineCascadedOperation(cstr,deps);
end
);

#for creating cascade permutations by giving dependency function maps in a list of argument-value pairs (the default dependency function value is () )
# CAUTION! This uses the new DepFuncTable functionality!
InstallOtherMethod(DefineCascadedOperation,[IsCascadedStructure, IsList],
function(cstr,pairs)
...
#for creating cascade permutations by giving dependency function maps in a dependency function table
InstallGlobalFunction(CascadedOperation,
function(cstr,depfunctable)
  local  depfunc;

  #a function that returns the value corresponding to the argument if found, otherwise the identity
  #------------------------embedded function------------------------------------------------ 
...
#the inverse of DefineCascadedOperation : returns a list containing coordinateprefixes - component element pairs 
#(when it is not the identity of the component)
#note that this returns concrete states now
InstallGlobalFunction(DependencyMapsFromCascadedOperation,
function(cascop)
local pairs,i,coords,cstr,identity, value;
...
#simple trick for getting the inverse #TODO to be replaced by a real calculation
InstallOtherMethod(InverseOp, "inverse of a cascaded permutation through flattening", true,
    [IsCascadedPermutation], 0,
function(cascperm)
    return BuildNC(CascadedStructureOf(cascperm),Inverse(Collapse(cascperm)));
end
);
...
###
#equality
# the worst case is when p and q are indeed equal, as every value is checked 
InstallOtherMethod(\=, "deciding equality of cascaded operations", IsIdenticalObj,
	[IsCascadedOperation, IsCascadedOperation], 0,
function(p,q)
local coords,i,cstr;
...
InstallOtherMethod(\<, "comparison of cascaded operations through flattening", IsIdenticalObj,
	[IsCascadedOperation, IsCascadedOperation], 0,
function(p,q)
...
InstallOtherMethod(OneOp, "neutral element for cascaded operations", true,
	[IsCascadedOperation], 0,
function(co)
...
###
#multiplication of cascaded operations - it is a tricky one, since it just combines together the functions
...
local depfunct,cstr;
  ...
  # constructing the dependency function of acting by q on arguments premoved by p
  # one dependency function is enough for all levels
  
  #--------------------------embedded function---------------------------------------
  depfunct := function(args)
    local  newargs;
      newargs := List([1..Length(args)], x-> SGPDEC_ActionOnLastCoord(args{[1..x]},p));
      #the product operation is the action of p on the arguments multiplied by the action of q on the p-moved args
      return p![Length(args)+1](args) * q![Length(args)+1](newargs);
   end;#depfunct
   #-----------------------------------------------------------------------------------

  return  Objectify(cstr!.operation_type,
                    ListWithIdenticalEntries(Length(cstr), depfunct)); #the new dependencies
end
);
...
#######################################
#exponent
InstallOtherMethod(\^, "exponent of  cascaded operation", true,
	[IsCascadedOperation, IsInt], 0,
function(p,n)
local i,result,multiplier;
...
InstallMethod( Display,
    "displays a cascaded operation",
    true,
    [IsCascadedOperation], 0,
function( co )
local i,j,cstr,comps;
...
InstallMethod( ViewObj,
    "displays a cascaded product operation",
    true,
    [IsCascadedOperation], 0,
function( co )
...
InstallOtherMethod(\^, "acting on cascaded states", true,
	[IsAbstractCascadedState, IsCascadedOperation], 0,
function(cs,co)
...
#############################################################################################
#YEAST for operations
# flattening - making an ordinary permutation out of the cascaded one
# the components are ordered then we do the enumeration then record as a transformation
InstallMethod(Collapse,
    "flattens a cascaded operation",
    true,
    [IsCascadedOperation], 0,
function( co )
local i,l,states,nstate,cstr;
  cstr := CascadedStructureOf(co);
  states := States(cstr);
...
local states, src_int,dest_cs,coordval,actionlist,level;  
  states := States(cstr);
  actionlist := [];
...
InstallOtherMethod(Build,
    "gives the cascade state representation of a permutation/transformation",
    true,
    [IsCascadedStructure,IsObject], 0,
function(cstr,flatop)
 
  #if it is not compatible 
  if not IsDependencyCompatible(cstr,flatop) then Print("Flatop is not in the wreath product!"); return fail; fi;                  
...
local cstr,arg,arguments,value,coord;
  ...
  for arg in arguments do  #TODO some optimization may be possible here, not to check any twice
    ...
    for coord in StateSets(cstr)[onlevel] do
      ...
      if value <>  cascop![targetlevel](arg) then return true; fi; 
    od;
  od;
  return false;
end);
...
function(cascop)
local graph,i,j,cstr;

  #getting the info record as it contains the componentinfo
  cstr := CascadedStructureOf(cascop);

  graph := [];
  for i in [2..Length(cstr)] do #checking all directed pairs systematically
    for j in [1..i-1] do
      if DependsOn(cascop,i,j) then Add(graph,[i,j]); fi;	  
    od;
  od;
...
###SCOPE#####################################
#returns a list of levels where  the given  cascaded operation has nontrivial action(s)
...
InstallMethod(CascadedStructureOf,"for cascaded operations",true,[IsCascadedOperation], 
function(cascop)
...
function(co)
local out, dep, deps, edges, vertices,coord, coordsname, newnn, edge, vertex, level, castruct, vertexlabels,i,str;
  ...
  HTAdd(vertexlabels,coordsname ," [width=0.2,height=0.2,label=\"\"]"); #adding a default label
  Sort(vertices);Sort(edges); #just to be on the safe side
  for dep in deps do
    ...
    for coord in dep[1] do
      ...
      if not (edge in edges) then  Add(edges,edge, PositionSorted(edges,edge)); fi; # we just add the full edge
    ...
    HTUpdate(vertexlabels,
             coordsname,
             Concatenation(" [label=\"", StringPrint(castruct!.operation_symbol_functions[level](dep[2])),"\"]"));
  od;

  # finally writing them into the dot file
  for i in [1..Size(vertices)] do  AppendTo(out, vertices[i]," ",HTValue(vertexlabels,vertices[i]),";\n"); od;
  for edge in edges do AppendTo(out, edge,";\n"); od;
