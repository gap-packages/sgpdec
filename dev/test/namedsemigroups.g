...
# variables of frequently used semigroups
...
# creates a wrapping function from the list of symbols for (semi)group states (points to act on)
...
#flip-flop
FlipFlop := SemigroupByGenerators([Transformation([1,1]),Transformation([2,2]),Transformation([1,2])]);
SetName(FlipFlop,"FF");
...
#the automaton with nonimage tiles - the name comes from a beer brand, and a restaurant in Eger
becks := [
...
Transformation([4,4,4,4,5,5]), #this and the nontrivial holonomy group of {4,5,6} generate the images with cardinality 2
...
#simple automaton demonstrating nonisomorphic permutator semigroups on equivalent subsets
nonisomperm := [ Transformation( [ 2, 1, 3 ] ), Transformation( [ 3, 2, 3 ] ), Transformation( [ 1, 1, 2 ] ) ];
NONISOMPERM := Semigroup(nonisomperm);
bex := [
Transformation([1,2,1,1]), #creates the image {1,2}
Transformation([4,4,4,3]), #transposition in {3,4}
Transformation([3,3,4,4]), #this and the nontrivial holonomy group of {4,5,6} generate the images with cardinality 2
...
smlbug := [ Transformation([2,2,4,4,6,6]), Transformation([1,3,3,4,5,3]), Transformation([1,2,1,2,1,2])  ];
