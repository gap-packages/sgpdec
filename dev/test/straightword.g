...
local t,w,rw,i;
    ...
    for i in [1..10] do
        ...
        if Size(w) <> 0 then Print(" reduced by ",100 - SGPDEC_Percentage(Size(Compacted(rw)),Size(w))," %\n\c"); fi; 
        if (not IsStraightWord(w,gens,One(gens[1]))) and (Length(rw) >= Length(w)) then
        ...
        if Construct(w,gens,One(gens[1]),\*) <> Construct(rw,gens,One(gens[1]),\*) then
