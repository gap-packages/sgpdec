...
#the official SDP cut out from the library
SDP := function( R, map, S )

    local P, genP, genR, genS, L3, perm, r, s, ordS, genno, LS, info, aut, res;
    res := rec(genGonN := [], genG := [], genN := []);
    ...
    od;
    res.genGonN := StructuralCopy(genP);
